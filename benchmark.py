...
try:
    ...
    print('>>>>>> loading deepspeed.profiling.flops_profiler successfully!')
    has_deepspeed_profiling = True
except ImportError as e: has_deepspeed_profiling = False
...
torch.backends.cuda.matmul.allow_tf32 = True ##only for A100
...
def cuda_timestamp(sync=False, device=None):
    if sync: torch.cuda.synchronize(device=device)
...
class BenchmarkRunner:
    def __init__(
        ...
        if 'cuda' in self.device: self.time_fn = partial(cuda_timestamp, device=self.device)
        else: self.time_fn = timestamp

    def _init_input(self):
        self.example_inputs = torch.randn((self.batch_size,) + self.input_size, device=self.device, dtype=self.data_dtype)
        if self.channels_last: self.example_inputs = self.example_inputs.contiguous(memory_format=torch.channels_last)
...
class InferenceBenchmarkRunner(BenchmarkRunner):
    ...
    def run(self):
        def _step():
            ...
            with self.amp_autocast(): output = self.model(self.example_inputs)
        ...
        with torch.no_grad():
            self._init_input()
            for _ in range(self.num_warm_iter): _step()
        ...
        retries = 0 if self.scripted else 2
        while retries:
            retries -= 1
            try:
...
class TrainBenchmarkRunner(BenchmarkRunner):
    ...
    def __init__(
        ...
        if kwargs.pop('grad_checkpointing', False): self.model.set_grad_checkpointing()

    def _gen_target(self, batch_size):
        return torch.empty((batch_size,) + self.target_shape, device=self.device, dtype=torch.long).random_(self.num_classes)
    ...
    def run(self):
        def _step(detail=False):
            ...
            with self.amp_autocast():
                ...
                if detail: t_fwd_end = self.time_fn(True)
                target = self._gen_target(output.shape[0])
                self.loss(output, target).backward()
                if detail: t_bwd_end = self.time_fn(True)
            self.optimizer.step()
...
class ProfileRunner(BenchmarkRunner):
    ...
    def __init__(self, model_name, device='cuda', profiler='', **kwargs):
        ...
        if not profiler:
            if has_deepspeed_profiling: profiler = 'deepspeed'
    ...
    def run(self):
        ...
        macs = 0
        activations = 0
        if self.profiler == 'deepspeed': macs, _ = profile_deepspeed(self.model, self.input_size, batch_size=self.batch_size, detailed=True)
        results = dict(
...
def benchmark(args):
    ...
    for prefix, bench_fn in zip(prefixes, bench_fns):
        ...
        if prefix and 'error' not in run_results: run_results = {'_'.join([prefix, k]): v for k, v in run_results.items()}
...
def main():
    ...
    if len(model_cfgs):
        ...
        if 'train' in args.bench: sort_key = 'train_samples_per_sec'
        elif 'profile' in args.bench: sort_key = 'infer_gmacs'
        results = filter(lambda x: sort_key in x, results)
        results = sorted(results, key=lambda x: x[sort_key], reverse=True)
        if len(results): write_results(results_file, results)
    else:
        results_file = args.results_file or './benchmark.txt'
        results = benchmark(args)
        texts = json.dumps(results, indent=4)
...
def write_results(results_file, results):
    with open(results_file, mode='w') as cf:
        ...
        dw.writeheader()
        for r in results: dw.writerow(r)
        cf.flush()
